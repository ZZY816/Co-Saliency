...
class Encode(nn.Module):
    ...
    def forward(self, input):
        ...
        #feature_map[-1] = F.normalize(feature_map[-1],p=1, dim= 1)
        return feature_map
...
class Recycle(nn.Module):
    ...
    def forward(self, feature_map_list, gates, multi):
        ...
        #print(a.size())
            activation = F.interpolate(feature_map_list[i][-1], size=(224, 224), mode='bilinear', align_corners=True)
        ...
        for i in range(num):
            ...
            #feature_map_list[i][-1] = feature_map_tensor[i]*similarity[i]

            out_list.append(self.decode(feature_map_copy))

        return  out_list, similarity, activation_list
...
class Mynet(nn.Module):
    ...
    def forward(self, input, gates,sals, mode):
        ...
        if mode =='test':
            for ii in range(6):
                #print(gates, i)
                out, similarity, activation = self.recycle(feature_map_list, gates_list, 1)
                gates_list = out
        ...
        return out_list_6, similarity_list, activation_list
